...
def initialize_state():
    ...
    if "biomarker_filter" not in st.session_state:
        st.session_state.biomarker_filter = "All"
    
    if "protocol_filter" not in st.session_state:
        st.session_state.protocol_filter = None
    ...
    if "settings_section" not in st.session_state:
        st.session_state.settings_section = "API Configuration"
...
# Horizontal Navigation with HTML/CSS
st.markdown(f"""
<div style="display: flex; align-items: center; gap: 1rem; padding: 1rem 2rem; background: rgba(255, 255, 255, 0.95); border-bottom: 1px solid {PRIMARY_COLOR}20; margin: -3rem -3rem 2rem -3rem; position: sticky; top: 0; z-index: 999;">
    <div style="font-weight: 800; font-size: 1.25rem; color: {PRIMARY_COLOR}; display: flex; align-items: center; gap: 0.5rem; min-width: 200px;">🏥 {BRAND_NAME}</div>
    <div style="display: flex; gap: 0.5rem; overflow-x: auto; flex: 1; scrollbar-width: none;">
""", unsafe_allow_html=True)
...
for i, opt in enumerate(nav_options):
    with nav_cols[i]:
        ...
        btn_style = f"background: {PRIMARY_COLOR}; color: white;" if is_active else f"background: transparent; color: #475569; border: 1px solid transparent;"
...
if st.session_state.current_page == "Executive Dashboard":
    st.markdown(f"""
    <div style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 2rem; background: rgba(22, 163, 74, 0.05); border-bottom: 1px solid {PRIMARY_COLOR}20; margin: -3rem -3rem 1.5rem -3rem; overflow-x: auto; scrollbar-width: none;">
        <div style="font-weight: 700; font-size: 0.9rem; color: {PRIMARY_COLOR}; min-width: fit-content;">📊 Dashboard Views: Overview | Key Metrics | Trends | Recommendations | Quick Actions</div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown(f"""
...
# --- DOCUMENT INTELLIGENCE ---
if st.session_state.current_page == "Document Intelligence":
    st.markdown(f"""
    <div style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 2rem; background: rgba(22, 163, 74, 0.05); border-bottom: 1px solid {PRIMARY_COLOR}20; margin: -3rem -3rem 1.5rem -3rem; overflow-x: auto; scrollbar-width: none;">
        <div style="font-weight: 700; font-size: 0.9rem; color: {PRIMARY_COLOR}; min-width: fit-content;">📁 Supported: PDF | Word | Excel | Images | JSON | Markdown | XML</div>
    </div>
    """, unsafe_allow_html=True)
...
# --- SPECIALIST CONSULTATION ---
    ...
    # Sub-menu for specialists
    st.markdown(f"""
    <div style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 2rem; background: rgba(22, 163, 74, 0.05); border-bottom: 1px solid {PRIMARY_COLOR}20; margin: -3rem -3rem 1.5rem -3rem; overflow-x: auto; scrollbar-width: none;">
        <div style="font-weight: 700; font-size: 0.9rem; color: {PRIMARY_COLOR}; min-width: fit-content;">👥 Select Specialist:</div>
    """, unsafe_allow_html=True)
    
    spec_names = list(SPECIALISTS.keys())
    spec_cols = st.columns(len(spec_names))
    
    for i, spec_name in enumerate(spec_names):
        with spec_cols[i]:
            spec_info = SPECIALISTS[spec_name]
            is_active = st.session_state.active_specialist == spec_name
            btn_type = "primary" if is_active else "secondary"
            
            if st.button(f"{spec_info['icon']} {spec_name}", key=f"spec_btn_{i}_{spec_name}", use_container_width=True, type=btn_type):
                if st.session_state.active_specialist != spec_name:
...
# --- BIOMARKER LABORATORY ---
    ...
    # Sub-menu for biomarker categories
    biomarker_categories = ["All", "Metabolic", "Lipids", "Hormones", "Thyroid", "Vitamins", "Inflammation", "Performance"]
    
    st.markdown(f"""
    <div style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 2rem; background: rgba(22, 163, 74, 0.05); border-bottom: 1px solid {PRIMARY_COLOR}20; margin: -3rem -3rem 1.5rem -3rem; overflow-x: auto; scrollbar-width: none;">
        <div style="font-weight: 700; font-size: 0.9rem; color: {PRIMARY_COLOR}; min-width: fit-content;">🧬 Biomarker Categories:</div>
    """, unsafe_allow_html=True)
    
    bio_cols = st.columns(len(biomarker_categories))
    for i, category in enumerate(biomarker_categories):
        with bio_cols[i]:
            is_active = st.session_state.biomarker_filter == category
            btn_type = "primary" if is_active else "secondary"
            if st.button(category, key=f"bio_cat_{i}_{category}", use_container_width=True, type=btn_type):
                if st.session_state.biomarker_filter != category:
                    st.session_state.biomarker_filter = category
                    st.rerun()
...
# --- WELLNESS PROTOCOLS ---
    ...
    st.markdown(f"""
    <div style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 2rem; background: rgba(22, 163, 74, 0.05); border-bottom: 1px solid {PRIMARY_COLOR}20; margin: -3rem -3rem 1.5rem -3rem; overflow-x: auto; scrollbar-width: none;">
        <div style="font-weight: 700; font-size: 0.9rem; color: {PRIMARY_COLOR}; min-width: fit-content;">📋 Protocol Categories</div>
    </div>
    """, unsafe_allow_html=True)
...
# --- HEALTH ANALYTICS ---
    ...
    st.markdown(f"""
    <div style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 2rem; background: rgba(22, 163, 74, 0.05); border-bottom: 1px solid {PRIMARY_COLOR}20; margin: -3rem -3rem 1.5rem -3rem; overflow-x: auto; scrollbar-width: none;">
        <div style="font-weight: 700; font-size: 0.9rem; color: {PRIMARY_COLOR}; min-width: fit-content;">📊 Analytics Views: Overview | Trends | Correlations | Comparisons | Predictions | Reports</div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown(f"""
    <div class="main-header">
...
# --- SETTINGS ---
    ...
    st.markdown(f"""
    <div style="display: flex; align-items: center; gap: 0.5rem; padding: 0.75rem 2rem; background: rgba(22, 163, 74, 0.05); border-bottom: 1px solid {PRIMARY_COLOR}20; margin: -3rem -3rem 1.5rem -3rem; overflow-x: auto; scrollbar-width: none;">
        <div style="font-weight: 700; font-size: 0.9rem; color: {PRIMARY_COLOR}; min-width: fit-content;">⚙️ Settings: API | Profile | Data | Preferences | About</div>
    </div>
    """, unsafe_allow_html=True)
    st.markdown(f"""
    <div class="main-header">
