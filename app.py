...
# --- SPECIALIST CONSULTATION ---
    ...
    spec_cols = st.columns(len(SPECIALISTS))
    for i, (spec_name, spec_info) in enumerate(SPECIALISTS.items()):
        with spec_cols[i]:
            is_active = st.session_state.active_specialist == spec_name
            if st.button(f"{spec_info['icon']} {spec_name}", key=f"spec_{spec_name}", use_container_width=True):
...
# --- BIOMARKER LABORATORY ---
    ...
    for i, category in enumerate(biomarker_categories):
        with bio_cols[i]:
            if st.button(category, key=f"bio_cat_{category}", use_container_width=True):
