...
# --- SPECIALIST CONSULTATION ---
    ...
    # Use selectbox for reliable specialist switching
    col1, col2, col3 = st.columns([2, 3, 2])
    with col2:
        selected = st.selectbox(
            "Choose Specialist:",
            options=spec_names,
            index=spec_names.index(st.session_state.active_specialist),
            key="specialist_selectbox",
            label_visibility="collapsed"
        )
        if selected != st.session_state.active_specialist:
            st.session_state.active_specialist = selected
            st.rerun()
